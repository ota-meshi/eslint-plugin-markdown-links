...
class DocFile {
  ...
  public updateHeader() {
    ...
    if (deprecated) {
      if (replacedBy) {
        const replacedRules = replacedBy.map(
          (name) => `[markdown-links/${name}](${name}.md) rule`,
        );
        notes.push(
    ...
    if (fixable) {
      notes.push(
        "- 🔧 The `--fix` option on the [command line](https://eslint.org/docs/latest/use/command-line-interface#fix-problems) can automatically fix some of the problems reported by this rule.",
      );
    }
    if (hasSuggestions) {
      notes.push(
...
This rule was introduced in eslint-plugin-markdown-links ${await this.since}
...
- [Rule source](https://github.com/ota-meshi/eslint-plugin-markdown-links/blob/main/src/rules/${ruleName}.ts)
- [Test source](https://github.com/ota-meshi/eslint-plugin-markdown-links/blob/main/tests/src/rules/${ruleName}.ts)
- [Test fixture sources](https://github.com/ota-meshi/eslint-plugin-markdown-links/tree/main/tests/fixtures/rules/${ruleName})
