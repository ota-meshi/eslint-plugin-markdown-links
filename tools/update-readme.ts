...
let insertText = `\n${renderRulesTableContent(
  2,
  (name) =>
    `https://ota-meshi.github.io/eslint-plugin-markdown-links/rules/${name}.html`,
)}\n`;
if (isWin) {
...
fs.writeFileSync(
  docsReadmeFilePath,
  newReadme
    .replace("# eslint-plugin-markdown-links\n", "# Introduction\n")
    .replace(
    ...
    .replace(
      /\[LICENSE\]\(\.\/LICENSE\)/gu,
      "[LICENSE](https://github.com/ota-meshi/eslint-plugin-markdown-links/blob/main/LICENSE)",
    )
    .replace(/<!--DOCS_IGNORE_START-->[\s\S]*?<!--DOCS_IGNORE_END-->/gu, "")
    .replace(
      /\(https:\/\/ota-meshi.github.io\/eslint-plugin-markdown-links/gu,
      "(.",
    )
    .replace(/\n{3,}/gu, "\n\n"),
  "utf8",
